...
function agentInputClosedReason(agent: Agent): string | undefined {
  ...
  if (stdin.destroyed || stdin.closed || stdin.writableDestroyed || stdin.writableEnded || stdin.writableFinished) {
...
function normalizeInputError(agent: Agent, err: any): Error {
  ...
  if (isBrokenInputError(err)) {
    return new Error(`Agent '${agent.id}' input stream is closed (${err.code}). The agent process likely exited; start a new test session and try again.`);
  }
...
export function writeAgentCommand(agent: Agent, command: Record<string, any>): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      const acceptsCallback = typeof stdin.write === "function" && stdin.write.length >= 2;
...
export async function sendToAgent(agent: Agent, message: string, timeoutMs: number, signal?: AbortSignal): Promise<void> {
  ...
  const perform = async () => {
    ...
    try {
      ...
      await writeAgentCommand(agent, cmd);
