...
import type { OrchestratorLibrariesInfo, ResourcePathValidation, ResourceScopeSettings, ResourceSettingsInfo } from "../../types.js";
...
export function OrchestratorLibrariesPanel({ pushLog, onDisplaySettingsChanged, onNativeSettingsSaved }: { pushLog: (text: string, level?: LogLine["level"]) => void; onDisplaySettingsChanged: () => void; onNativeSettingsSaved: () => void }) {
  ...
  const [bootstrapTargetPath, setBootstrapTargetPath] = useState("./.pi/orchestrator-library");
  ...
  const bootstrapLibrary = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await responseErrorText(res));
      const result = await res.json() as { scope?: "global" | "project"; library?: { root: string; manifest?: { name: string } } };
  ...
  return <div className="space-y-4">
    <Card>
      <CardHeader className="border-b border-border"><div className="flex items-center justify-between gap-3"><CardTitle>Orchestrator Libraries</CardTitle><div className="flex gap-2"><Button variant="secondary" onClick={openBootstrapDialog}>+ New Library</Button><Button variant="secondary" onClick={load} disabled={loading}>Refresh</Button></div></div></CardHeader>
      <CardContent className="space-y-2 pt-4 text-sm text-muted-foreground">
        ...
        {data && !data.libraries.length && <button type="button" className="w-full rounded-md border border-dashed border-border p-6 text-center transition hover:border-primary/70 hover:bg-primary/5" onClick={openBootstrapDialog}>
          <div className="font-medium text-foreground">Click here to scaffold your first Orchestrator Library</div>
          <div className="mt-1">Pi can create a starter library for your agent types, templates, root profiles, skills, and extensions instead of requiring manual settings edits.</div>
        </button>}
      </CardContent>
    </Card>
    <Dialog open={creatingLibrary} title="Scaffold Orchestrator Library" onOpenChange={(open) => !bootstrapSaving && setCreatingLibrary(open)} className="max-w-3xl">
      <form className="space-y-3" onSubmit={bootstrapLibrary}>
        <p className="text-sm text-muted-foreground">Choose an explicit folder for the starter library. A path inside this repo uses project settings; outside this repo uses global settings.</p>
        {bootstrapError && <div className="rounded-md border border-destructive/50 bg-destructive/10 p-2 text-sm text-destructive">{bootstrapError}</div>}
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-1"><FieldLabel required>Target path</FieldLabel><Input value={bootstrapTargetPath} onChange={(e) => setBootstrapTargetPath(e.target.value)} placeholder="./.pi/orchestrator-library" /></div>
          <div className="space-y-1"><FieldLabel optional>Library name</FieldLabel><Input value={bootstrapName} onChange={(e) => setBootstrapName(e.target.value)} placeholder="team-ai" /><FormMessage>Used as the namespaced resource prefix; leave blank to derive it from the folder name.</FormMessage></div>
        </div>
        <div className="space-y-1"><FieldLabel optional>Description</FieldLabel><Textarea rows={3} value={bootstrapDescription} onChange={(e) => setBootstrapDescription(e.target.value)} placeholder="Shared team orchestrator resources." /></div>
        <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3"><Button type="submit" disabled={bootstrapSaving}>{bootstrapSaving ? "Creating…" : "Create library"}</Button><Button type="button" variant="secondary" onClick={() => setCreatingLibrary(false)} disabled={bootstrapSaving}>Cancel</Button></div>
      </form>
    </Dialog>
