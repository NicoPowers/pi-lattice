...
import type { AgentTypeInfo, AgentTypeTestSession, ModelInfo } from "../../types.js";
...
export function AgentTypesPanel({ types, onNew, onEdit, onTest, large }: { types: AgentTypeInfo[]; onNew: () => void; onEdit: (type: AgentTypeInfo) => void; onTest?: (type: AgentTypeInfo) => void; large?: boolean }) {
  return (
        ...
        {!types.length ? <p className="text-sm text-muted-foreground">No agent types found.</p> : <div className="grid gap-3 md:grid-cols-2">
          {types.map((type) => (
            <div key={type.name} className="rounded-md border border-border p-3">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="truncate text-sm font-semibold">{type.name}</span>{type.agentClass && <Badge variant="outline">{type.agentClass}</Badge>}</div><div className="mt-1 line-clamp-3 text-xs text-muted-foreground">{type.description}</div></div><div className="flex shrink-0 gap-1"><Button variant="secondary" className="px-2 py-1 text-xs" onClick={() => onTest?.(type)}>Test</Button><Button variant="secondary" className="px-2 py-1 text-xs" onClick={() => onEdit(type)}>Edit</Button></div></div>
            </div>
          ))}
        </div>}
      </CardContent>
...
export function AgentTypeTestDialog({ open, typeDef, onClose, pushLog }: { open: boolean; typeDef?: AgentTypeInfo; onClose: () => void; pushLog?: (text: string, level?: "info" | "success" | "error") => void }) {
  ...
  useEffect(() => {
    ...
    setServerError("");
    setBusy(true);
    fetch(`/api/agent-types/${encodeURIComponent(typeDef.name)}/test-session`, { method: "POST" })
      .then(async (res) => {
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data?.error || "Failed to start test session");
        if (cancelled) {
          if (data?.session?.id) await fetch(`/api/agent-type-test-sessions/${encodeURIComponent(data.session.id)}`, { method: "DELETE" }).catch(() => {});
          return;
        }
  ...
  return <Dialog open={open} title={typeDef ? `Test ${typeDef.name}` : "Test Agent Type"} onOpenChange={close} closeOnBackdrop={false} closeOnEscape={false} className="max-w-4xl">
    <div className="space-y-3">
      <div className="rounded-md border border-border bg-background p-3 text-xs text-muted-foreground">
        <div>Status: <span className="text-foreground">{session?.status || (busy ? "starting" : "not started")}</span></div>
        {session?.worktree && <div>Worktree: <code>{session.worktree}</code></div>}
        {diagnostics && <div>Runtime tools: {diagnostics.active.length} active / {diagnostics.all.length} total{diagnostics.conflicts?.length ? `, ${diagnostics.conflicts.length} conflicts` : ""}</div>}
        {!!diagnostics?.active.length && <div>Active: {diagnostics.active.map((tool) => tool.name).join(", ")}</div>}
        {!!diagnostics?.conflicts?.length && <div className="text-destructive">Conflicts: {diagnostics.conflicts.map((conflict) => `${conflict.name} (${conflict.sources.join(", ")})`).join("; ")}</div>}
      </div>
      <div className="max-h-80 space-y-2 overflow-auto rounded-md border border-border bg-background p-3">
        {messages.length ? messages.map((entry, index) => <div key={index} className="text-sm"><span className="font-semibold capitalize text-muted-foreground">{entry.role}: </span><span className="whitespace-pre-wrap">{entry.text}</span></div>) : <div className="text-sm text-muted-foreground">Starting disposable session…</div>}
      </div>
      {session?.stderrTail && <pre className="max-h-32 overflow-auto whitespace-pre-wrap rounded-md border border-border bg-background p-2 text-xs text-destructive">{session.stderrTail}</pre>}
      <ValidationSummary errors={[]} serverError={serverError} />
      <div className="flex gap-2"><Textarea rows={3} value={message} onChange={(e) => setMessage(e.target.value)} disabled={!session || busy} /><Button onClick={send} disabled={!session || busy || !message.trim()}>{busy && session ? "⏳ Sending…" : "Send"}</Button></div>
      <div className="flex justify-end"><Button variant="secondary" onClick={close}>Close & Cleanup</Button></div>
    </div>
  </Dialog>;
}
